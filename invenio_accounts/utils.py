...
def jwt_authorization_header(headers):
    ...
    """
    code = headers.get(
        current_app.config['ACCOUNTS_JWT_AUTH_HEADER']
    )
    if code is None:
...
def jwt_verify_token(headers):
    ...
    authentication_type = current_app.config['ACCOUNTS_JWT_AUTH_HEADER_TYPE']
    if authentication_type is not None:
        # Get the prefix and the token
        prefix, token = token.split()
        # Check if the type matches
        if prefix != current_app.config['ACCOUNTS_JWT_AUTH_HEADER_TYPE']:
            raise JWTInvalidHeaderError
    # Get the token data
